...
import { Bell, CheckCheck, CircleX, ImageIcon, InfoIcon, MessageSquare, X } from 'lucide-react'
                  ...
                  <div className='flex items-center justify-between w-full'>
                    <div className='flex items-center gap-3'>
                      {notification.imageUrl ? (
                        <img src={notification.imageUrl} alt='notification' className='w-10 h-10 rounded-md' />
                      ) : (
                        <span className='text-lg flex-shrink-0'>{getNotificationIcon(notification.type)}</span>
                      )}

                      <div className='flex flex-col items-start'>
                        <span className={cn('font-medium text-sm', !notification.read && 'text-blue-900')}>
                          {notification.title}
                        </span>

                        <div className='flex-1 min-w-0 flex items-center gap-1 mt-1'>
                          {!notification.read && <div className='size-2 bg-blue-500 rounded-full flex-shrink-0' />}
                    ...
                    <p className='text-xs text-gray-400'>{notification.timestamp.toLocaleString().split(' ')[1]}</p>
                  </div>
                </div>
              ))}
            </div>
