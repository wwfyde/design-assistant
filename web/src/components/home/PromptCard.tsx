...
export function PromptCard({ prompt }: PromptCardProps) {
  ...
  const handleUseImage = (url: string, text?: string | null) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <Card className='group relative overflow-hidden rounded-xl border-0 shadow-none hover:shadow-lg transition-all duration-300 bg-gray-200 p-3'>
        <div className='relative overflow-hidden rounded-xl'>
          {prompt.image && (
            <img
              src={prompt.image}
              alt={prompt.title || 'Prompt Image'}
              className='w-full h-auto max-h-[450px] object-cover transition-transform duration-300 group-hover:scale-105'
              loading='lazy'
            />
          )}
            ...
            <div className='p-8 md:p-10 flex-1 overflow-y-auto'>
              ...
              <div className='space-y-4'>
                      ...
                      <div className='flex gap-2'>
                        ...
                        {prompt.image && (
                          <Button
                            ...
                            size='sm'
                            className='h-8 rounded-full text-xs px-3 gap-1.5 hover:bg-primary hover:text-primary-foreground transition-colors'
                            onClick={() => handleUseImage(prompt.image!, prompt.prompt_zh)}
                          >
                ...
                {prompt.prompt_en && (
                  <>
                    <div className='flex items-center justify-between'>
                      <span className='text-xs font-bold text-muted-foreground tracking-wider uppercase'>Prompt</span>
                      <div className='flex gap-2'>
                        <Button
                          variant='outline'
                          size='sm'
                        ...
                        {prompt.image && (
                          <Button
                            ...
                            size='sm'
                            className='h-8 rounded-full text-xs px-3 gap-1.5 hover:bg-primary hover:text-primary-foreground transition-colors'
                            onClick={() => handleUseImage(prompt.image!, prompt.prompt_en)}
                          >
                            <ImageIcon className='w-3.5 h-3.5' />
                            参考作图
                          </Button>
