...
const CanvasCard: React.FC<CanvasCardProps> = ({ index, canvas, handleCanvasClick, handleDeleteCanvas }) => {
  ...
  return (
    ...
    >
      <CanvasDeleteDialog show={showDeleteDialog} setShow={setShowDeleteDialog} handleDeleteCanvas={handleDelete}>
        <Button
          variant='secondary'
          size='icon'
      ...
      <div className='p-3 flex flex-col gap-2' onClick={() => handleCanvasClick(canvas.id)}>
        {canvas.thumbnail ? (
          <img src={canvas.thumbnail} alt={canvas.name} className='w-full h-40 object-cover rounded-lg' />
        ) : (
