...
const ModelSelectorV3: React.FC<ModelSelectorV3Props> = ({ onModelToggle, onAutoToggle }) => {
  ...
  const handleAutoToggle = (enabled: boolean) => {
    ...
    if (enabled) {
      ...
      if (selectedToolsList.length > 0) {
        ...
        localStorage.setItem(
          'disabled_tool_ids',
          JSON.stringify(allTools.filter((t) => !selectedToolsList.includes(t)).map((t) => t.id)),
        )
      }
    }
    setAutoMode(enabled)
      ...
      <DropdownMenuTrigger asChild>
        <Button
          ...
          className={`w-fit max-w-[40%] justify-between overflow-hidden ${
            autoMode ? 'bg-background border-border text-muted-foreground' : 'text-primary border-green-200 bg-green-50'
          }`}
        >
          {autoMode ? (
            <svg
              ...
              return (
                <DropdownMenuGroup key={provider}>
                  <DropdownMenuLabel className='text-xs font-medium text-muted-foreground px-0 py-2'>
                    <div className='flex items-center gap-2'>
                      <img src={providerInfo.icon} alt={providerInfo.name} className='w-4 h-4 rounded-full' />
                      {providerInfo.name}
                    </div>
                  </DropdownMenuLabel>
