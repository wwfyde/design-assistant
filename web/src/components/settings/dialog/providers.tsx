  ...
  return (
    <div className='flex flex-col items-center justify-center p-4 w-full sm:pb-0 pb-10 relative'>
      ...
      {!isLoading &&
        Object.keys(providers).map((key, index) => (
          <div key={key} className='w-full'>
            {key === 'jaaz' ? (
              <JaazSetting config={providers[key]} onConfigChange={handleConfigChange} />
            ) : key === 'comfyui' ? (
              <ComfyuiSetting config={providers[key]} onConfigChange={handleConfigChange} />
            ) : (
              <CommonSetting
                providerKey={key}
            ...
            {index !== Object.keys(providers).length - 1 && <div className='my-6 border-t bg-border' />}
          </div>
        ))}
      ...
      <div className='flex fixed bottom-0 left-[calc(var(--sidebar-width))] gap-1 right-0 px-1'>
        ...
        <Button variant='outline' onClick={() => setIsAddProviderDialogOpen(true)} className='w-1/2' size='lg'>
