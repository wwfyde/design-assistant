    ...
    <Dialog open={open} onOpenChange={setShowSettingsDialog}>
      ...
      >
        <SidebarProvider className='h-[calc(100vh-60px)]! min-h-[calc(100vh-60px)]! flex-1 relative'>
          <SettingSidebar current={current} setCurrent={setCurrent} onClose={() => setShowSettingsDialog(false)} />
          <ScrollArea className='max-h-[calc(100vh-50px)]! w-full'>{renderContent()}</ScrollArea>
        </SidebarProvider>
      </CommonDialogContent>
    </Dialog>
  )
