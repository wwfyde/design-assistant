...
export default function CommonSetting({ providerKey, config, onConfigChange, onDeleteProvider }: CommonSettingProps) {
    ...
    <div className='space-y-4'>
      ...
      <div className='space-y-2'>
        ...
        <Input
          ...
          className='w-full'
        />
        <p className='text-xs text-gray-500'>{t('settings:provider.apiKeyDescription')}</p>
      </div>
      ...
      {hasMaxTokens && (
        <div className='space-y-2'>
          <Label htmlFor={`${providerKey}-maxTokens`}>{t('settings:provider.maxTokens')}</Label>
          <Input
