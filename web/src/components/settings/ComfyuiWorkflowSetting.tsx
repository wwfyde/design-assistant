...
import { PaletteIcon, PencilIcon, PlusIcon, TrashIcon, UploadIcon } from 'lucide-react'
  ...
  const loadWorkflows = async () => {
    fetch('/api/settings/comfyui/list_workflows')
      ...
      .then((data: ComfyWorkflowFromAPI[]) => {
        ...
        const workflows: ComfyWorkflow[] = data.map((workflow: ComfyWorkflowFromAPI) => {
      ...
      {deleteWorkflowId && (
        <Dialog open={!!deleteWorkflowId} onOpenChange={() => setDeleteWorkflowId(null)}>
          <DialogContent>
            ...
            <Button onClick={() => handleDeleteWorkflow(deleteWorkflowId)}>Delete</Button>
          </DialogContent>
        </Dialog>
      )}
      {
        <div className='flex items-center gap-2'>
          ...
          <Button variant='outline' size='sm' onClick={() => setShowAddWorkflowDialog(true)}>
        ...
        <div className='space-y-2'>
          <div className='grid grid-cols-2 gap-2'>
            {workflows.map((workflow) => (
              <div key={workflow.id} className='flex items-center gap-2 border p-2 rounded-md justify-between'>
...
function AddWorkflowDialog({ onClose, workflow }: { onClose: () => void; workflow: ComfyWorkflow | null }) {
  const { t } = useTranslation()
  const inputRef = useRef<HTMLInputElement>(null)
  const [workflowName, setWorkflowName] = useState(workflow?.name ?? '')
  const [workflowJson, setWorkflowJson] = useState<Record<string, ComfyUIAPINode> | null>(workflow?.api_json ?? null)
  const [inputs, setInputs] = useState<ComfyWorkflowInput[]>(workflow?.inputs ?? [])
      ...
      try {
        ...
        toast.error(
          t(
            ...
            'Invalid workflow JSON, make sure you exprted API JSON in ComfyUI!' + error,
          ),
        )
      }
    }
  ...
  const handleSubmit = async () => {
    if (!workflowJson) {
      setError(t('settings:comfyui.pleaseUploadWorkflowApiJsonFile', 'Please upload a workflow API JSON file'))
      return
    }
    if (inputs.length === 0) {
      setError(t('settings:comfyui.pleaseAddAtLeastOneInput', 'Please add at least one input'))
      return
    }
    if (workflowName === '') {
      setError(t('settings:comfyui.pleaseEnterWorkflowName', 'Please enter a workflow name'))
      return
    }
    if (workflowDescription === '') {
      setError(t('settings:comfyui.pleaseEnterWorkflowDescription', 'Please enter a workflow description'))
      return
    }
    const matched = workflowName.match(/^[a-zA-Z0-9_]+$/)
    if (!matched) {
      setError(t('settings:comfyui.workflowNameInvalid', 'Workflow Name only allow a-Z, 0-9, _'))
      return
    }
    const payload = {
      name: workflowName,
    ...
    if (workflow) {
      // 先删除
      const deleteRes = await fetch(`/api/settings/comfyui/delete_workflow/${workflow.id}`, {
        method: 'DELETE',
      })
      if (!deleteRes.ok) {
    ...
    if (createRes.ok) {
      toast.success(
        t('settings:comfyui.workflowUpdatedSuccessfully', `Workflow ${workflow ? 'updated' : 'created'} successfully`),
      )
      onClose()
    } else {
      const data = await createRes.json()
      toast.error(`Failed to ${workflow ? 'update' : 'create'} workflow: ${data.message}`)
    }
  }
  return (
    <Dialog
      ...
      >
        ...
        <div className='flex-1 overflow-y-auto space-y-3 pr-2'>
          <Input
            type='text'
            style={{ flexShrink: 0 }}
            placeholder={t('settings:comfyui.workflowName', 'Workflow Name, only a-Z, 0-9, _ are allowed')}
            value={workflowName}
            onChange={(e) => setWorkflowName(e.target.value)}
          />
          <Textarea
            placeholder={t('settings:comfyui.workflowDescription', 'Workflow Description')}
            value={workflowDescription}
            onChange={(e) => setWorkflowDescription(e.target.value)}
          />
          <Button onClick={() => inputRef.current?.click()} variant={'outline'}>
            <UploadIcon className='w-4 h-4 mr-2' />
            {t('settings:comfyui.uploadWorkflowApiJson', 'Upload Workflow API JSON')}
          </Button>
          <input type='file' accept='.json' ref={inputRef} onChange={handleFileChange} className='hidden' />
          {workflowJson && (
            <Card className='border-2 border-dashed border-primary/20'>
              <CardHeader>
                <CardTitle className='flex items-center'>
                  <div className='h-2 w-2 rounded-full bg-primary'></div>
                  {t('settings:comfyui.workflowInputsConfiguration', 'Workflow Inputs Configuration')}
                </CardTitle>
              </CardHeader>
                          ...
                          <div className='flex items-start justify-between gap-4'>
                            <div className='flex-1 space-y-3'>
                              ...
                              <div className='flex items-center gap-2 flex-wrap'>
                                <Label className='font-mono text-sm bg-muted px-2 py-1 rounded'>{input.name}</Label>
                              ...
                              <div className='space-y-2'>
                                <Label className='text-xs text-muted-foreground'>
                                  {t('settings:comfyui.defaultValue', 'Default Value')}
                                </Label>
                                <Input
                                  type='text'
                  ...
                  <Card className='border-dashed border-2 border-muted-foreground/25'>
                    <CardContent className='flex flex-col items-center justify-center py-12 text-center'>
                      ...
                      <h3 className='font-medium text-foreground mb-2'>
                        {t('settings:comfyui.noInputsConfigured', 'No inputs configured')}
                      </h3>
                    ...
                    {Object.keys(node.inputs).map((inputKey) => {
                      ...
                      return (
                        <div key={inputKey} className='flex items-center gap-2'>
                          <p className='bg-accent text-sm px-2 py-0.5 rounded-md'>{inputKey}</p>
                          <Input type='text' value={inputValue.toString()} disabled />
                          <Button
                            variant='outline'
                            size='default'
                            onClick={() => {
                              setInputs([
                                ...inputs.filter((i) => i.node_id !== nodeID || i.node_input_name !== inputKey),
                                {
                                  name: `${inputKey}_${nodeID}`,
                                  type: typeof inputValue as 'string' | 'number' | 'boolean',
                                  description: '',
                                  node_id: nodeID,
        ...
        <div className='flex-shrink-0 mt-2 pt-3 border-t border-border bg-background'>
          <Button onClick={handleSubmit} className='w-full'>
            {workflow ? t('settings:comfyui.save', 'Save') : t('settings:comfyui.submit', 'Submit')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
