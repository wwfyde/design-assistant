    ...
    const loadModelInfo = async () => {
      ...
      try {
        ...
        if (result.success && result.has_metadata) {
          ...
          if (!extractedModel) {
            // 检查是否有参数字段包含模型信息
            const paramKeys = ['parameters', 'Parameters', 'params', 'generation_params']
            for (const key of paramKeys) {
              if (metadata[key] && typeof metadata[key] === 'string') {
                const paramStr = metadata[key]
                // 尝试从参数字符串中提取模型名
                const modelMatch = paramStr.match(/(?:model|Model):\s*([^,\n]+)/i)
                if (modelMatch) {
...
export default function MaterialManager() {
  ...
  // 获取图片尺寸的函数
  const getImageDimensions = useCallback((imagePath: string): Promise<{ width: number; height: number }> => {
  ...
  const loadFolder = useCallback(
    async (path: string = '') => {
      ...
      try {
        ...
        setFolderContents((prev) => new Map(prev).set(result.current_path, result.items))
    ...
    async (file: FileSystemItem) => {
      ...
      if (file.type === 'image') {
        ...
        if (file.path.toLowerCase().endsWith('.png')) {
          try {
            const pngMetadata = await readPNGMetadata(getFileServiceUrl(file.path))
  ...
  const handleAddToChat = useCallback((file: FileSystemItem, event?: React.MouseEvent) => {
    if (event) {
  ...
  const getFileIcon = useCallback((type: string, className: string = 'w-4 h-4') => {
    switch (type) {
      case 'folder':
  ...
  const filteredMediaFiles = mediaFiles.filter((file) => file.name.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  const getFilteredFolderContents = useCallback(
    (path: string): FileSystemItem[] => {
      ...
      return searchTerm ? searchInFolderContents(contents, searchTerm) : contents
    },
      ...
      return items.map((item) => (
        <div key={item.path} className={`select-none`}>
          <div
            ...
            onClick={() => (item.is_directory ? handleFolderClick(item) : handlePreviewFile(item))}
          >
            {item.is_directory && (
            ...
            {!item.is_directory && item.size && (
              <span className='text-xs text-gray-400 ml-2'>{formatFileSize(item.size)}</span>
            )}
          </div>
        ...
        {filteredMediaFiles.map((file) => (
          ...
          >
            <div className='w-12 h-12 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0 relative'>
              ...
              {file.type === 'image' && file.name.toLowerCase().endsWith('.png') && (
                <div className='absolute -top-1 -right-1 z-10'>
                  <ImageModelBadge filePath={file.path} fileName={file.name} variant='list' />
                </div>
              )}
            ...
            <div className='flex-1 min-w-0 overflow-hidden'>
              ...
              <div className='text-sm text-gray-500 flex items-center gap-4 overflow-hidden'>
                <span className='whitespace-nowrap'>{formatFileSize(file.size || 0)}</span>
                <span className='whitespace-nowrap'>{formatDate(file.mtime)}</span>
              </div>
            </div>
    ...
    return (
      <div className='absolute bottom-0 left-0 right-0 mt-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            ...
            <div className='aspect-video bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden flex items-center justify-center'>
              {selectedFile.type === 'image' ? (
                ...
                <div className='flex flex-col items-center justify-center text-gray-400'>
                  ...
                  <span className='text-sm mt-2'>{selectedFile.type.toUpperCase()}</span>
                </div>
              )}
            </div>
          ...
          <div className='space-y-4'>
            <div>
              <h4 className='font-medium mb-2'>{t('canvas:basicInfo', 'Basic info')}</h4>
              <div className='space-y-2 text-sm'>
                <div className='flex justify-between'>
                  <span className='text-gray-600 dark:text-gray-400'>{t('canvas:fileName', 'File name')}</span>
                  <span className='font-medium break-all'>{selectedFile.name}</span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-gray-600 dark:text-gray-400'>{t('canvas:fileSize', 'File size')}</span>
                  <span className='font-medium'>{formatFileSize(selectedFile.size || 0)}</span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-gray-600 dark:text-gray-400'>{t('canvas:modifiedTime', 'Modified time')}</span>
                  <span className='font-medium'>{formatDate(selectedFile.mtime)}</span>
                </div>
                {selectedFile.dimensions && (
                  <div className='flex justify-between'>
                    <span className='text-gray-600 dark:text-gray-400'>{t('canvas:dimensions', 'Dimensions')}</span>
            ...
            <div>
              <h4 className='font-medium mb-2'>{t('canvas:filePath', 'File path')}</h4>
            ...
            {selectedFile.pngMetadata && selectedFile.pngMetadata.success && selectedFile.pngMetadata.has_metadata && (
              <div>
                <h4 className='font-medium mb-2'>{t('canvas:pngMetadata', 'PNG Metadata')}</h4>
                <div className='space-y-2 text-sm max-h-64 overflow-y-auto bg-gray-50 dark:bg-gray-700 p-3 rounded'>
                  {Object.entries(selectedFile.pngMetadata.metadata)
                    .filter(([key]) => !['width', 'height', 'mode'].includes(key)) // 过滤掉基本信息
                    .map(([key, value]) => (
  ...
  }, [selectedFile, getFileIcon, formatFileSize, formatDate, handleAddToChat, handlePreviewFile])
    ...
    <div className='flex h-full bg-gray-50 dark:bg-gray-900 w-full overflow-hidden'>
      ...
      <div className='w-80 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col flex-shrink-0'>
        ...
        <div className='p-4 border-b border-gray-200 dark:border-gray-700'>
          {/* My Assets Button */}
          <Button
            variant={selectedFolder?.path === myAssetsPath.current ? 'default' : 'ghost'}
          ...
          <div className='flex items-center gap-2 mb-3 px-[4px]'>
            ...
            <div className='flex-1 text-sm text-gray-600 dark:text-gray-400 truncate'>{currentPath || '~'}</div>
            <button
        ...
        <div className='flex-1 overflow-y-auto p-2'>
          ...
          {!loading && !error && <div className='space-y-1'>{renderFileTree(filteredItems)}</div>}
        </div>
      </div>

      {/* Right Panel */}
      <div className='flex-1 flex flex-col min-w-0 overflow-hidden relative' onClick={() => setSelectedFile(null)}>
        {/* Header */}
            ...
            <div>
              <div className='flex items-center gap-2'>
                <h2 className='text-2xl font-bold text-gray-900 dark:text-white'>
                  {selectedFolder ? selectedFolder.name : t('canvas:selectAFolder', 'Select a folder')}
                </h2>
                {selectedFolder && (
                  <button
          ...
          {selectedFolder ? (
              ...
              {filteredMediaFiles.length > 0 ? (
                ...
                <div className='flex items-center justify-center h-64 text-gray-500'>
                  <div className='text-center'>
                    ...
                    <p className='text-lg font-medium'>{t('canvas:noMediaFiles', 'No media files')}</p>
                    <p className='text-sm mt-2'>
                      {t('canvas:noMediaFilesDescription', 'No images or videos in this folder')}
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className='flex items-center justify-center h-full text-gray-500'>
              <div className='text-center'>
                ...
                <p className='text-xl font-medium'>{t('canvas:selectAFolder', 'Select a folder')}</p>
                <p className='text-sm mt-2'>
                  {t('canvas:selectAFolderDescription', 'Select a folder to view its images and videos')}
                </p>
              </div>
            </div>
          )}
